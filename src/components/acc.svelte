<script>
    ...
    function loginWithEmail () {
        if(orgCode){
            dispatch(type, {name: submitName, email: email, password: password, orgCode: orgCode});
        } else {
            dispatch(type, {name: submitName, email: email, password: password, tMail: tMail});
        }
    }
...
<Row>
    <Col>
        <div>
            ...
            {#if user == null}
                <Button id = "submitButton" outline color= "success" class = "float-right" on:click = {loginWithEmail} size = "block">Submit</Button>
            {/if}
        </div>
    </Col>
</Row>
...
<Row>
    <Col style="text-align: center">
        <Button on:click={showTeacher} block id="accTypeButton"> Make a teacher account </Button>
    </Col>
</Row>
