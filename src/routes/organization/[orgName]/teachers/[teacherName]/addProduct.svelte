<script>
    ...
    async function submit () {
        ...
        console.log(name)
        if (name && price) {
            var add = await db.collection("organization").doc($page.params.orgName).collection("teachers").doc($page.params.teacherName).collection("products").add({name: name, price: price, userId: $authStore.userId})
            if (picture[0].type === "image/png" || picture[0].type === "image/jpeg") {
                var pic = picture[0]
                const mainPicturePath = `/${$authStore.userId}/${name}.${pic.name.split(".").pop()}`;
    ...
    onMount(async () => {
        var db = firebase.firestore()
        var urlTeacher = db.collection("organization").doc($page.params.orgName).collection("teachers").doc($page.params.teacherName)
        var snapshot = await urlTeacher.get()
        if (!snapshot.exists){
            alert("This teacher name does not exist, you will be redirected to the home page")
...
<h2>Add a product to {$page.params.teacherName}</h2> <br>
...
{#if sucessMessage!= ""} 
    ...
    <center><a href={`/organization/${$page.params.orgName}`}><h2 class="link"> Click here to return to the home page </h2></a></center>
{/if}