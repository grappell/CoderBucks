<script>
  ...
  onMount(async () => {
    ...
    await teacherEmails.forEach(async (email) => {
      ...
      promiseArr.push(getTeacherProductsWithEmail(email, orgName));
    });

    await Promise.all(promiseArr)
      .then((data) => {
        console.log("then", data);
        teacherProductSuperlist = teacherProductSuperlist.concat(data);
      })
      .catch((data) => {
        displayPopup("danger", data, "Problem feching student products", false);
      });
  ...
  <section class="photo-grid">
    ...
    >
      ...
      {#if openStore}
        ...
        <!-- was working here -->
        <div
          style="font-size: 20px; overflow-y:scroll; overflow-x:hidden;"
          class="flex-row"
        >
